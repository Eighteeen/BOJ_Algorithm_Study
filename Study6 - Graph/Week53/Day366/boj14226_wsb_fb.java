...
class Main {
    ...
    //// => 변경해봤습니다.
    static int getMinTimeOfMakeToEmoticon(int screenEmoticon, int wantEmoticon) {
        initForMakeToEmoticon(screenEmoticon, wantEmoticon);

        while (!bfsQueue.isEmpty()) {
            Coordinate current = bfsQueue.poll();
            int currentScreen = current.getX();
            int currentClipboard = current.getY();
            if (currentScreen == wantEmoticon) {
