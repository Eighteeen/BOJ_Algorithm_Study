...
class Main {
  ...
  static int getOptimalTimeToGetEmoji(int requiredEmojis) {
    ...
    while (queue.size() > 0) {
      ...
      //// => 피드백 감삼다!
      boolean canCopy = !visited[current.emojis][current.emojis];
      if (canCopy) {
        queue.add(new TypingInfo(current.emojis, current.emojis, current.requiredTime + 1));
      }
      boolean canPaste = current.emojis + current.clips <= 1000 && !visited[current.clips][current.emojis + current.clips];
      if (canPaste) {
        queue.add(new TypingInfo(current.clips, current.emojis + current.clips, current.requiredTime + 1));
      }
      boolean canRemove = current.emojis >= 1 && !visited[current.clips][current.emojis - 1];
      if (canRemove) {
