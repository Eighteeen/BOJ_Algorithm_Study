...
class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (true) {
            ...
            if (member1 == -1) break;
            setMemberFriendsOneScore(member1, member2);
        }

        setMemberScoresByFloydWarshall();
    ...
    //// => 반영했습니다.
        //// 사용한 알고리즘을 명시하면 어떻게 점수가 설정되는 건지 알 수 있기 때문에 함수명에 알고리즘명을 명시했고,
        //// throughKScore는 애초에 변수명을 잘못 지어서 변경하고, 조건도 boolean변수로 의미를 명확히 해봤습니다.
    static void setMemberScoresByFloydWarshall() {
        for (int k = 1; k <= memberSize; k++) {
            for (int i = 1; i <= memberSize; i++) {
                for (int j = 1; j <= memberSize; j++) {
                    int scoreThroughK = memberScores[i][k] + memberScores[k][j];
                    boolean isLowerScoreThroughK = scoreThroughK < memberScores[i][j];
                    if (isLowerScoreThroughK) {
                        memberScores[i][j] = scoreThroughK;
                    }
                }
            }
        }
    ...
    //// => 반영했습니다.
    static void setMemberFriendsOneScore(int member1, int member2) {
