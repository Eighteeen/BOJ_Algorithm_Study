...
//// 괜찮은걸요 : 22 로직도 전체적으로 깔끔하고 괜찮은데요?
//// => 감사합니다ㅠㅠ
const [N, M] = input().split(' ').map(Number);
//// 친구다 1 아직아니다 0인거같은데 대신 true, false로 표현했다면 더 직관적이였을 것 같슴다
//// : 22 js 상에선 1을 true 처럼 사용할 수 있지만 boolean을 직접 사용하는 게 직관적일 것 같아요.
//// => 이 부분은 인접행렬이기 때문에 0과 1이 더 적합하다고 생각해요! 고로 바꾸지 않겠습니다.
...
while (cntNumOfFriends < N * (N - 1)) {
  ...
  //// newCntNumOfFriends 식으로 지었으면 정확했을 것 같습니다! : 22
  //// => 반영했습니다.~
  let newCntNumOfFriends = 0;
  ...
  for (let me = 1; me <= N; me++) {
    for (let myFriends = 1; myFriends <= N; myFriends++) {
      if (!friendsGraph[me][myFriends] && friendsGraphOfFriends[me][myFriends]) {
        cntNumOfFriends += 1;
        newCntNumOfFriends += 1;
      }
      friendsGraph[me][myFriends] = friendsGraphOfFriends[me][myFriends];
    }
  }

  newFriendsNumOnDays.push(parseInt(newCntNumOfFriends / 2));
}
