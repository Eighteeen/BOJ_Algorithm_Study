...
class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        //// -> 반영했습니다.
        List<Coordinate> puddle = new ArrayList<>();
        ...
        makePuddleMap(puddle, minX, minY);

        System.out.println(
            getMinDistanceAvoidPuddle(
                //// 요 함수가 어떤 역할을 하는지 알기 어려웠어요. 좌표 두개를 넣으면 좌표가 나온다? 어떤 좌표가 나온다는거지?
                //// -> 함수명은 있는 그대로 해석하면 되기 때문에 매개변수명을 바꿔서 입력값을 좀 더 명확히 해 혼동이 가지 않도록 해봤어요.
    ...
    static Coordinate getCoordinateInPuddleMap(int totalMapX, int totalMapY, int totalMapMinX, int totalMapMinY) {
        return Coordinate.twoPointOf(totalMapX - totalMapMinX, totalMapY - totalMapMinY);
    }
