...
class Main {
    public static void main(String[] args) throws IOException {
        ...
        for (int byteMessage : byteMessages) {
            sb.append(unscrambleByteMessage(byteMessage)).append(" ");
        }
    ...
    //// -> 반영했습니다.
    static int unscrambleByteMessage(int byteMessage) {
        int unscrambledNum = 0;

        while (scrambleByteMessage(unscrambledNum) != byteMessage) {
            unscrambledNum++;
        }
    ...
    static int scrambleByteMessage(int num) {
        int shiftNum = num << 1;
