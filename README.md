...
* 사이트 이름, 문제번호와 본인의 이니셜로 소스파일 이름을 지정한다.
  * [백준](https://www.acmicpc.net) - 기본 알고리즘 문제 사이트
    ...
    * 모범 답안: bj0000_rank
* 문제가 2시간 이상 풀리지 않는다면 포기할 수 있다.
  * 대신 다음 날까지 아래 내용의 PDF를 작성하여 올린다.
    > 1. 본인이 짠 코드 이미지
    > 2. 본인이 시도한 로직 설명
    > 3. 모범답안의 로직을 어떻게 이해했는지 설명
    > 4. 모범답안에 한줄한줄 주석달아서 구현 이해한 코드 이미지
    > 5. 자신의 코드가 안 됐던 이유 분석 
    > * 그 외 하고싶은 말 (앞으로는 이렇게 해야겠다 등등)
