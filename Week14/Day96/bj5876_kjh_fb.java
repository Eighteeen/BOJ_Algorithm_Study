...
class ShoesBoard {
  ...
  //// => 위의 상수로 커버하려했던 의도였는데
  //// => isOpening, isClosing 함수를 봐야만 알 수 있어서 가독성이 떨어졌네요
  //// => 좋은 의견 고마워요!
  private boolean[][] isLeftShoes;
  private int size;

  public ShoesBoard(int size) {
    isLeftShoes = new boolean[size][size];
    this.size = size;
  }

  public void addShoesInRow(int row, String shoesRow) {
    for (int i = 0; i < shoesRow.length(); i++) {
      isLeftShoes[row][i] = shoesRow.charAt(i) == '(';
    }
  }

  public int getMaxBalanced() {
    if (isClosing(isLeftShoes[0][0])) return 0;
  ...
  private int getMaxBalanced(int row, int col, boolean[][] visited, int opening, int closing) {
    ...
    if (closing > 0 && isOpening(isLeftShoes[row][col])) return 0;

    visited[row][col] = true; 
    opening += isOpening(isLeftShoes[row][col]) ? 1 : 0;
    closing += isClosing(isLeftShoes[row][col]) ? 1 : 0;
