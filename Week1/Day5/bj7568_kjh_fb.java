...
class Main {
  public static void main(String[] args) throws Exception {
    ...
    //// => 이 경우는 아래아랫줄에서 WEIGHT와 HEIGHT가 바로 쓰이기때문에 시선의 이동을 줄이기 위해 붙여놓는 쪽이 더 좋은 것 같아요! (제 취향상)
    for(int i = 0; i < N; i++) {
    ...
    //// 입력받으면서 처리할 수 있는게 좋은거 같아요 이코드는.
      ...
      //// => 피드백 감사합니다! 제 스스로 느끼기에도 가독성이 아쉽네요 ㅠㅠ
      //// => '덩치가 더 작으면 순위를 뒤로 보낸다'로 한번 표현을 바꿔봤습니다!
      for(int j = 0; j < i; j++) {
        boolean iIsSmaller = (bodies[i][WEIGHT] < bodies[j][WEIGHT])
        ...
        boolean jIsSmaller = (bodies[j][WEIGHT] < bodies[i][WEIGHT])
                              && (bodies[j][HEIGHT] < bodies[i][HEIGHT]);
        if (jIsSmaller) {
          ranks[j] += 1;
        }
      }
