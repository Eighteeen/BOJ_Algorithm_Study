...
class Main {
  ...
  static int getLeastMove(int distance) {
    ...
    //// 변수가 어디로 날라갔을까요?
    //// => 변수이름이 마음에 안들어서 바꿨는데 다 바꾸질 못했네요 ㅎㅎ; 수정했슴다
    double andThenRoundDown = Math.floor(rootAndMultiplyTwo);

    //// 변수가 어디로 날라갔을까요..?
    // 다만 1 4 9 등 제곱수에서는 1 크게 구해져서 그땐 -1을 해줌
    if (Double.compare(rootAndMultiplyTwo, andThenRoundDown) == 0) {
      return (int) andThenRoundDown - 1;
    }
    return (int) andThenRoundDown;
  }
}
