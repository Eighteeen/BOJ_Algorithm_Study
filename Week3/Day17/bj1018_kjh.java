...
class Main {
  public static void main(String[] args) throws Exception {
    ...
    int minForBoards = ROWS * COLUMNS;
    for (int i = 0; i < rowCompareCount; i++) {
      for (int j = 0; j < columnCompareCount; j++) {
        int minForCurrentBoard = calcMinForABoard(chessBoard, i, j);
        if (minForCurrentBoard < minForBoards) minForBoards = minForCurrentBoard;
      }
    }
    System.out.print(minForBoards);
  }
  ...
  private static int calcMinForABoard(char[][] chessBoard, int startRowIndex, int startColumnIndex) {
    ...
    for (int i = startRowIndex; i < startRowIndex + 8; i++) {
      for (int j = startColumnIndex; j < startColumnIndex + 8; j++) {
        ...
        if (isNeedRepaintOppositeStandard) needRepaintsOppositeStandard++;
      }
    }
