...
class Main {
  ...
  static int getMaxPathDistance(Node root) {
    ...
    for (Edge childEdge : root.childrenEdges) {
      ...
      childrenMaxDistances.add(distance + getMaxPathDistance(child));
    }

    childrenMaxDistances.sort(Comparator.reverseOrder());

    // 자식의 maxDistance 중 가장 큰 두 가지를 합한 것이
    // 현 노드를 중간지점으로 거치는 것 중 최대 지름(maxDiameter)
