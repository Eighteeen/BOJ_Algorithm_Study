...
//// => 함수이름에 대해 너무 깊게 생각하다보니 더 불필요한 요소가 들어간 것 같네요,,
//// => isTree는 변수도 똑같이 선언하였기 떄문에 isTreeNode로 반영하겠습니다.
const isTreeNode = (startNode, tree, visitedNodeList) => {
  let isTree = true;
  const stackNodeList = [startNode];
...
while (true) {
  ...
  for (let i = 1; i <= N; i++) {
    ...
    if (isTreeNode(i, tree, visitedNodeList)) treeCnt += 1;
  }
