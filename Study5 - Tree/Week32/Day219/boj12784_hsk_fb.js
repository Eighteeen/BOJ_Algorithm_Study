...
for (let i = 0; i < T; i++) {
  ...
  for (let j = 1; j < numOfIslands; j++) {
    ...
    //// => 오오오옥 맞아요~ 변경했어요!
    const [nodeA, nodeB, numOfDynamite] = input().split(' ').map(Number);

    tree[nodeA].push({ connectedNode: nodeB, numOfDynamite });
    tree[nodeB].push({ connectedNode: nodeA, numOfDynamite });
  }
