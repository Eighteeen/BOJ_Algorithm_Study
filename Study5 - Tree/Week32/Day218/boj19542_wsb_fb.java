...
class NodeMap {
    ...
    public int getRoundDistance(int rootData, int distanceNotGoing) {
        ...
        //// -> 저는 반환에 대한 내용이 앞에 있는 게 더 자연스러워 보여서
        //// -> Setting 대신에 Doing set이라고 변경하여서 ~을 하면서 라는 의미를 더욱 명확히 해봤습니다.
        getDepthFromLeafDoingSetRoundDistance(root);
        return roundDistance * 2;
    }

    //// 저는 roundDistance를 따로 계산했는데 한번에 하니까 더 보기 쉬운 것 같아요
    private int getDepthFromLeafDoingSetRoundDistance(Node node) {
        if (node.isLeafNode()) return 0;
        
        int depthFromLeaf = 0;
        for (Node child : node.getAdjacentNodeList()) {
            child.removeAdjacentNode(node);
            int currentDepthFromLeaf = getDepthFromLeafDoingSetRoundDistance(child) + 1;
