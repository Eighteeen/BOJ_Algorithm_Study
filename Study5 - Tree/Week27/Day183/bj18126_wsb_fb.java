...
class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println(getMaxDistance(rootNode));
        br.close();
    }

    //// 갱신되는 최대값은 return되는 값으로 충분히 받을 수 있으니 untilDistance 매개변수는 없어도 괜찮을 것 같습니다
    //// -> 반영했어요 깔끔해졌네요
    static long getMaxDistance(Node fromNode) {
        long maxDistance = 0;
        ...
        for (Node adjacentNode : edgeDistanceMap.keySet()) {
            ...
            maxDistance = Math.max(maxDistance, edgeDistance + getMaxDistance(adjacentNode));
        }
